...
const fetchPainting = async (req, res, next) => {
  ...
  try {
    lists.nora = await Painting.find({ user: users[0].id }).limit(20).sort( { "upload_date": -1 } )
  }catch(err){
    console.log(err)
  }
  try {
    lists.elaine = await Painting.find({ user: users[1].id }).limit(20).sort( { "upload_date": -1 } )
  }catch(err){
    console.log(err)
  }
